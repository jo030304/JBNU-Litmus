<p align="center">
  <img src="site/resources/images/readme/readme-banner-litmus.png" alt="Litmus Online Judge" width="800px">
</p>

<p align="center">
  전북대학교 컴퓨터공학부를 위한 현대적인 온라인 저지 시스템
</p>

<div align="center">
    <a href="https://litmus.jbnu.ac.kr/">Home Page</a> |
    <a href="site/LICENSE">License</a> |
    <a href="https://github.com/DMOJ/online-judge">Based on DMOJ</a>
</div>

## 프로젝트 소개

Litmus는 [DMOJ](https://github.com/DMOJ/online-judge) 기반으로 개발된 전북대학교 컴퓨터공학부 온라인 저지 시스템입니다. 프로그래밍 과제, 알고리즘 문제 해결, 프로그래밍 대회 등을 지원합니다.

## 주요 기능

### 과제 · 대회 운영

- **과제(Assignments) / 대회(Contests) 분리 운영**: 용도별 순위 형식(과제용 · 대회용)과 오답 패널티 설정
- **지각 제출**: 과제에 한해 마감 이후 제출 기한을 별도 지정
- **실시간 순위표**: 시간 제한, 문제별 배점 관리
- **표절 검사**: [JPlag](https://github.com/jplag/JPlag) 연동 유사도 검사 및 리포트 확인
- **결과 내보내기**: 대회 결과 엑셀 다운로드, 참가자 제출 코드 일괄 다운로드

### 채점

- **지원 언어**: C, C++14/17/20, Java 8, Python 3, Text
- **테스트 케이스 기반 자동 채점**: 채점 데이터 파일 업로드, 테스트 케이스별 배점
- **분산 채점기**: 다중 채점기 연결 및 채점기 상태 페이지 제공

### 문제 관리

- **Markdown 기반 문제 지문**: LaTeX 수식(MathJax) 지원
- **문제 목록 CSV / 엑셀 다운로드**

### 계정 · 권한 · 보안

- **통합 로그인**: 자체 ID/PW 방식 및 JEduTools(Keycloak) SSO
- **로그인 보호**: 로그인 연속 실패 시 일정 시간 잠금
- **교수 · 조교(TA) 권한**: 문제 및 대회 단위 편집 권한 위임

### 사용자 환경

- **다크 모드**: 라이트 / 다크 / 시스템 기본값
- **패치 노트**: 서비스 변경 사항 안내

## 사용법

Litmus는 [litmus.jbnu.ac.kr](https://litmus.jbnu.ac.kr)에서 이용할 수 있습니다.

개발 및 운영 관련 내부 절차는 Wiki를 참고하세요.

## 스크린샷


### 메인 페이지

![Litmus 메인 페이지](site/resources/images/readme/screenshot1.png)

### 과제/대회 페이지

![과제 목록 페이지](site/resources/images/readme/screenshot2.png)
![과제 상세 정보 페이지](site/resources/images/readme/screenshot3.png)

### 문제 페이지

![문제 목록 페이지](site/resources/images/readme/screenshot4.png)
![문제 지문 페이지](site/resources/images/readme/screenshot5.png)

### 제출 페이지

![답안 제출 화면](site/resources/images/readme/screenshot6.png)
![테스트 케이스별 채점 결과 화면](site/resources/images/readme/screenshot7.png)

### 채점기 상태 페이지

![채점기 상태 페이지](site/resources/images/readme/screenshot8.png)


## 개발진

### 2024년

<a href="https://github.com/bootkorea"><img src="https://github.com/bootkorea.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/tjdeo1102"><img src="https://github.com/tjdeo1102.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/afpine"><img src="https://github.com/afpine.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/daeuning"><img src="https://github.com/daeuning.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/jys-jeong"><img src="https://github.com/jys-jeong.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/sooinji"><img src="https://github.com/sooinji.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/dameun2224"><img src="https://github.com/dameun2224.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/seoharu"><img src="https://github.com/seoharu.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/Jggyu"><img src="https://github.com/Jggyu.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/JC-arl"><img src="https://github.com/JC-arl.png" width="50" style="border-radius: 50%;"/></a>

### 2025년

<a href="https://github.com/dameun2224"><img src="https://github.com/dameun2224.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/jeong-gu"><img src="https://github.com/jeong-gu.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/Jggyu"><img src="https://github.com/Jggyu.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/JC-arl"><img src="https://github.com/JC-arl.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/rlathdns"><img src="https://github.com/rlathdns.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/les151"><img src="https://github.com/les151.png" width="50" style="border-radius: 50%;"/></a>

### 2026년

<a href="https://github.com/jeong-gu"><img src="https://github.com/jeong-gu.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/jo030304"><img src="https://github.com/jo030304.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/Mindungil"><img src="https://github.com/Mindungil.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/hcw889"><img src="https://github.com/hcw889.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/Eunho-sp"><img src="https://github.com/Eunho-sp.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/lia0519"><img src="https://github.com/lia0519.png" width="50" style="border-radius: 50%;"/></a>
<a href="https://github.com/xilver2918"><img src="https://github.com/xilver2918.png" width="50" style="border-radius: 50%;"/></a>

## 라이선스 및 오픈소스 고지

이 프로젝트는 [AGPL-3.0 License](site/LICENSE) 하에 배포됩니다.

Litmus는 다음 오픈소스를 활용합니다.

- [DMOJ](https://github.com/DMOJ/online-judge): 본 프로젝트의 기반이 되는 온라인 저지 시스템
- [JPlag](https://github.com/jplag/JPlag): 제출 코드 표절 검사

## 문의

- **관리자**: litmus@jbnu.ac.kr
- **소속**: 전북대학교 컴퓨터공학부 ALPS (Litmus 개발팀)
- **웹사이트**: [litmus.jbnu.ac.kr](https://litmus.jbnu.ac.kr)
