...
class PrivateProblemException(Exception):
    """문제가 존재하지만 비공개인 경우 발생하는 예외"""
    pass
...
class ProblemMixin(object):
    ...
    def no_such_problem(self):
        ...
        return generic_message(self.request, '존재하지 않는 문제',
                               '%s 문제를 찾을 수 없습니다.' % code, status=404)

    def private_problem(self):
        code = self.kwargs.get(self.slug_url_kwarg, None)
        return generic_message(self.request, '비공개 문제',
                               '%s 문제는 비공개이며 접근할 수 없습니다.' % code, status=404)
    ...
    def get(self, request, *args, **kwargs):
        ...
        except PrivateProblemException:
            return self.private_problem()
...
class ProblemDetail(ProblemMixin, SolvedProblemMixin, CommentedDetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # 암호화된 문제 처리
        if self.object.is_encrypted:
            context['title'] = self.object.name
            context['need_password'] = True
            context['description'] = ''

            minimal_context = context.copy()

            # 기본 context와 함께 필수 context 추가
            minimal_context.update({
                'title': context['title'],
                'description': context['description'],
                'need_password': True,
                'vote_perm': context['vote_perm'],
                'problem': self.object,
                'has_pdf_render': HAS_PDF,
                'comment_form': kwargs.get('comment_form'),
                'completed_problem_ids': self.get_completed_problems(),
                'contest_problem': None,
                'available_judges': Judge.objects.filter(online=True, problems=self.object),
                'show_languages': self.object.allowed_languages.count() != Language.objects.count(),
                'can_edit_problem': self.object.is_editable_by(user),
        ...
        if contest_problem or user.is_superuser:
            ...
            context['has_clarifications'] = clarifications.exists()
    ...
    def check_password(self, request, password):
        ...
        return {'status': 'error', 'message': '잘못된 비밀번호입니다.'}

@require_POST
def check_problem_password(request, problem):
    try:
        # CSRF 보호는 Django 미들웨어로 자동 처리됨
        ...
        try:
            # 암호화된 내용이 있는지 확인
            if not problem.encrypted_description:
                return JsonResponse({
                    'status': 'error',
    ...
    def get_contest_queryset(self):
        ...
        return [{
            ...
            'author':p['author'],
                                   ...
                                   'problem__group__full_name', 'points', 'partial', 'user_count','author')]
