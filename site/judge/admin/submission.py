...
from judge.models import ContestParticipation, ContestProblem, ContestSubmission, Profile, Submission, \
    SubmissionSource, SubmissionTestCase,Language
...
class CombinedSubmissionFilter(FieldListFilter):
    title = ' '
    template = 'admin/input_filter/input_filter_submission.html'  # 템플릿 따로 필요
    ...
    def __init__(self, field, request, params, model, model_admin, field_path):
        ...
        self.__status_lookups = (('None', _('None')), ('NotDone', _('Not done')), ('EX', _('Exceptional'))) + Submission.STATUS
        self.__status_handles = set(map(itemgetter(0), Submission.STATUS))

        self.__result_lookups = (('None', _('None')), ('BAD', _('Unaccepted'))) + Submission.RESULT
        self.__result_handles = set(map(itemgetter(0), Submission.RESULT))
    ...
    def language_lookups(self):
        return self.__language_lookups
    
    @property
    def status_lookups(self):
        return self.__status_lookups

    @property
    def result_lookups(self):
        return self.__result_lookups
    ...
    def queryset(self, request, queryset):
        ...
        if language and language in self.__language_handles:
            queryset = queryset.filter(language_id=language)

        if status == 'None':
            queryset = queryset.filter(status=None)
        elif status == 'NotDone':
            queryset = queryset.exclude(status__in=['D', 'IE', 'CE', 'AB'])
        elif status == 'EX':
            queryset = queryset.exclude(status__in=['D', 'CE', 'G', 'AB'])
        elif status in self.__status_handles:
            queryset = queryset.filter(status=status)

        if result == 'None':
            queryset = queryset.filter(result=None)
        elif result == 'BAD':
            queryset = queryset.exclude(result='AC')
        elif result in self.__result_handles:
            queryset = queryset.filter(result=result)

        if code:
            queryset = queryset.filter(problem__code__icontains=code)
